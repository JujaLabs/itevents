...
public interface IntegrationEventMapper extends IntegrationEventDao {
    ...
    @Results({
        ...
        @Result(property = "integrationEventUrl",
            column = "integration_event_url"),
        @Result(property = "event", column = "event_id", javaType = Event.class,
            one = @One(select =
                "org.itevents.dao.mybatis.mapper.EventMapper.getEvent"))

    })
    @Select("SELECT * FROM crawler WHERE integration_name=#{integrationName} " +
        "ORDER BY integration_event_url")
    List<IntegrationEvent> getIntegrationEventsByIntegrationName(
        String integrationName);

    @Override
    @Insert("INSERT " +
        "INTO crawler(integration_name, integration_event_url, event_id)" +
